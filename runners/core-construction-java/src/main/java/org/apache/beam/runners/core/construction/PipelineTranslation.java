...
public class PipelineTranslation {
  ...
  public static RunnerApi.Pipeline toProto(
    ...
    if (urnsToOverride.size() > 0) {
      ...
      final Map<
          ...
          knownTranslators = KnownTransformPayloadTranslator.KNOWN_PAYLOAD_TRANSLATORS;
      ...
      for (Entry<String, PTransform> entry : transforms.entrySet()) {
        ...
        if (!alreadyCheckedURns.contains(urn) && urnsToOverride.contains(urn)) {
          ...
          List<AppliedPTransform> appliedPTransforms =
              findAppliedPTransforms(urn, pipeline, knownTranslators);
          for (AppliedPTransform appliedPTransform : appliedPTransforms) {
            TransformPayloadTranslator payloadTranslator =
                knownTranslators.get(appliedPTransform.getTransform().getClass());
            try {
  ...
  private static RunnerApi.Pipeline updateTransformViaTransformService(
      String urn,
    ...
    try {
      ...
      for (TupleTag<?> tag : (Set<TupleTag<?>>) appliedPTransform.getInputs().keySet()) {
        input = input.and(tag, (PCollection) appliedPTransform.getInputs().get(tag));
      }
      POutput output = externalTransform.expand(input);
      ...
      if (output instanceof PCollectionTuple) {
        ...
        for (TupleTag<?> tag : newOutputs.keySet()) {
          if (!appliedPTransform.getOutputs().containsKey(tag)) {
            throw new RuntimeException(
                "Could not find the tag " + tag + " in the original set of outputs");
          }
          inputReplacements.put(originalOutputs.get(tag).getName(), newOutputs.get(tag).getName());
        }
          ...
          .forEach(
              (transformId, transform) -> {
                ...
                for (Map.Entry<String, String> entry : transform.getOutputsMap().entrySet()) {
                  if (inputReplacements.containsKey(entry.getValue()) && urn.equals(transform.getSpec().getUrn())) {
  ...
  private static List<AppliedPTransform> findAppliedPTransforms(
      String urn,
      Pipeline pipeline,
      Map<Class<? extends org.apache.beam.sdk.transforms.PTransform>, TransformPayloadTranslator>
          knownTranslators) {

    List<AppliedPTransform> appliedPTransforms = new ArrayList<>();
    pipeline.traverseTopologically(
        new PipelineVisitor.Defaults() {

          void findMatchingAppliedPTransform(Node node) {
            if (node.getTransform() != null
                && knownTranslators.containsKey(node.getTransform().getClass())) {
              TransformPayloadTranslator translator =
                  knownTranslators.get(node.getTransform().getClass());
              if (translator.getUrn() != null && translator.getUrn().equals(urn)) {
                appliedPTransforms.add(node.toAppliedPTransform(pipeline));
              }
            }
          }
